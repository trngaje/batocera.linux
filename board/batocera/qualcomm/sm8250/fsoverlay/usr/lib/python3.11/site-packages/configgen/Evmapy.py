...
########## drastic needs two different evmappy files for devices with only a single joystick or two
        if emulator == "drastic":
            with open('/boot/boot/batocera.board', 'r') as file:
                BOARD = file.read().strip()

            if BOARD in ["rg28xx", "rg40xx-v", "rg35xx-plus"]:
                source_file = "/usr/share/evmapy/nds.drastic.keys.singlejoystick"
            else:
                source_file = "/usr/share/evmapy/nds.drastic.keys.dualjoystick"

            destination_file = "/usr/share/evmapy/nds.drastic.keys"
