...
########## This is needed for devices with only a single joystick real or virtual
        with open('/boot/boot/batocera.board', 'r') as file:
            BOARD = file.read().strip().upper()

        valid_boards = ["RG28XX", "RG40XX-V", "RG35XX-PLUS"]

        if BOARD in valid_boards:
            file_path = "/usr/share/evmapy/nds.drastic.keys"

            with open(file_path, 'r') as file:
                data = json.load(file)

            removed_triggers = ["joystick1up", "joystick1down", "joystick1left", "joystick1right"]
            data['actions_player1'] = [action for action in data['actions_player1']
                                       if action['trigger'] not in [[trigger] for trigger in removed_triggers]]

            for action in data['actions_player1']:
                if action['trigger'] == "joystick2" and action['type'] == "mouse":
                    action['trigger'] = "joystick1"
                    print("Updated joystick2 to joystick1")
